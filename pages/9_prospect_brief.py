...
def _build_new_brief_prompt(
    ...
    return (
        ...
        f"**RULE 2b — CHANNEL DECIDES THE WEDGE (General Trade vs Modern Trade).** "
        f"Classify the prospect's route to market BEFORE choosing the wedge — the "
        f"default WhatsApp-ordering pitch is a GT play and is WRONG for modern trade:\n"
        f"  • **General Trade** (distributor / dealer / retailer network, kirana, FMCG "
        f"distribution) → wedge = WhatsApp-native ORDERING + scheme comms for "
        f"distributors/retailers (Sales & Ordering). Default only for GT.\n"
        f"  • **Modern Trade / large-format retail / D2C** (own retail chains, department "
        f"stores — e.g. Trent/Westside/Zudio, Shoppers Stop, Reliance Retail) → do NOT "
        f"pitch distributor WhatsApp-ordering. The wedge is a **Living Knowledge Graph "
        f"over their POS / order / loyalty data → clienteling, household intelligence, AI "
        f"product discovery, and 'what to send / when'** (Search & Discovery + Decision "
        f"Intelligence); buyer is CRM / Marketing / Merchandising, not GT sales. The "
        f"proven pattern is the **Trent and Shoppers Stop 'Living KG' demos** — frame the "
        f"wedge that way for modern-trade prospects.\n"
        f"  • If the prospect runs BOTH (a brand with GT distribution AND its own retail / "
        f"D2C), name both and say which is the sharper entry.\n\n"
